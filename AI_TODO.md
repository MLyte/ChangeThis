...
## Journal
- [2026-05-18] Diagnostic permission GitHub issue: transformation de l'erreur GitHub brute `Resource not accessible by personal access token` en message actionnable indiquant le dépôt cible et la permission fine-grained requise `Repository permissions > Issues > Read and write`; le formulaire token GitHub rappelle aussi `Repository access` + permission Issues. Validation ciblée: `npm run test --workspace @changethis/web -- issue-providers.test.ts` OK; `npm run typecheck --workspace @changethis/web` OK; `npm run lint --workspace @changethis/web` OK; `npm test` OK.
- [2026-05-18] Diagnostic CSP installation widget: le test serveur du script ne retourne plus de faux positif quand le snippet est present mais bloque par la Content Security Policy du site hote; il verifie `script-src`, `connect-src` et `style-src 'unsafe-inline'`, puis renvoie les directives a ajouter. Le script racine `npm test` build aussi le widget avant les tests web pour aligner CI et bundle public. Validation ciblee: `npm run test --workspace @changethis/web -- script-test-route.test.ts` OK; `npm run typecheck --workspace @changethis/web` OK; `npm run lint --workspace @changethis/web` OK; `npm test` OK.
- [2026-05-18] Durcissement sauvegarde Connexions Git: correction du regex UUID casse qui faisait basculer l'activation provider vers le store local au lieu du PATCH Supabase, ajout d'un `router.refresh()` apres mutation settings, et relecture serveur de l'integration avant de renvoyer `connected`. Objectif: supprimer les faux positifs ou exposer une erreur actionnable au lieu d'un retour UI fantome. Validation: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run test --workspace @changethis/web` OK, `npm run build` OK.
- [2026-05-18] Correctif relecture Connexions Git: la page settings considere maintenant `provider_integrations.status = connected` comme source d'etat persistante, afin qu'un token enregistre et verifie ne retombe pas visuellement a "No account linked" si l'hydratation de la liste des credentials est incomplete. Validation: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run test --workspace @changethis/web` OK, `npm run build` OK.
- [2026-05-18] Clarification UX Connexions Git: l'ecran indique maintenant explicitement qu'un token serveur deja connecte est enregistre chiffre et non reaffiche, avec une action "Remplacer le token" pour rotation/reconnexion. Validation: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run test --workspace @changethis/web` OK, `npm run build` OK.
...
- [2026-05-02] Port main vitrine: cherry-pick des derniers ajustements homepage depuis `codex/full-live-signup` vers `main`, en conservant le comportement waitlist de `main` sans CTA `/signup`/`/login`. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check --cached` OK.
...
- [2026-05-04] Synchronisation documentation vitrine: mise à jour du README et des docs état courant/review/readiness/MVP/quick-start pour refléter la home waitlist, l'architecture courte, la réassurance IT/DPO neutre, la charte couleur stricte et le positionnement GitHub/GitLab non priorisé. Validation ciblée: `git diff --check` OK avec avertissements CRLF existants.

...
## Journal
- [2026-05-19] Fallback ConfirmationURL anti-scanner: le template Supabase documente maintenant une URL ChangeThis contenant `confirmation_url={{ .ConfirmationURL | urlquery }}`; `/auth/confirm` affiche le bouton humain puis declenche l'URL officielle Supabase seulement au clic. Validation ciblee: typecheck web OK, tests web OK.
- [2026-05-19] Protection scanners email token_hash: `/auth/confirm` n'echange plus automatiquement un `token_hash` a l'ouverture du lien; il affiche un bouton humain `Finaliser mon acces`, puis redirige seulement au clic vers `/api/auth/callback`. Validation ciblee: typecheck web OK, tests web OK.
...
- [2026-05-19] Signup email-first: `/signup` ne demande plus le mot de passe et envoie un lien Supabase vers `/auth/confirm?next=/signup/set-password`; le mot de passe et le workspace sont crees apres verification e-mail. Ajout d'un template HTML Supabase Auth dans `docs/supabase-auth-email-template.fr.md`. Validation ciblee: typecheck web OK, lint web OK, tests web OK, build web OK, `git diff --check` OK.
- [2026-05-19] Recomposition dashboard projets: `/projects` passe en trois colonnes desktop (`sites connectes` en navigation gauche, inbox au centre, contexte a droite), avec retour deux colonnes sous 1320px et une colonne sous 980px. Les sites filtrent maintenant la file via `site=<publicKey>` avec item actif et compteurs. Validation ciblee: typecheck web OK, lint web OK, build web OK, `git diff --check` OK.
- [2026-05-19] Mise a jour reporter feedbacks existants: ajout d'une route publique bornee par origine/projet pour mettre a jour `payload.reporter`, regeneration du brouillon d'issue, et bouton `Mettre a jour le contact` dans `Gerer les feedbacks` quand les retours envoyes n'ont pas le contact courant. Libelle `Retirer l'envoi` remplace par `Retirer le feedback`. Validation ciblee: tests web/widget OK, typecheck web/widget OK, lint web OK, build widget OK, `git diff --check` OK.
- [2026-05-19] Micro-correction widget contact: les coordonnees reporter sont repliees derriere un bouton `Contact` avec icone mail et resume compact; les champs restent deployes si le contact est requis et manquant. Validation ciblee: `npm run test --workspace @changethis/widget` OK, `npm run build --workspace @changethis/shared` OK, `npm run typecheck --workspace @changethis/widget` OK, `npm run build --workspace @changethis/widget` OK.
...
- [2026-05-02] Ajout section vitrine contexte feedback: liste claire des informations conservees avec un retour (page, appareil/OS via navigateur, taille d'ecran, message, repere, capture, element cible, destination de traitement) avec version FR/EN et grille responsive. Validation lancee apres changement.
- [2026-05-02] Micro-correction copy vitrine: remplacement du titre beta abstrait (`vrai chemin du retour client vers l'action`) par une promesse plus concrete: capter un retour precis, le trier simplement, puis preparer la bonne tache. Validation lancee apres changement.
- [2026-05-02] Ajustement couleur secondaire vitrine ciment: remplacement du taupe `#72685f`, trop brun, par un ciment mineral plus froid (`#7d8280`) avec surface `#eef1ef`, texte graphite `#3f4543` et ombres alignees. Validation lancee apres changement.
- [2026-05-02] Ajustement exemple probleme vitrine: le troisieme contre-exemple passe de `Tache a reconstruire` a `Filtre manquant`, pour expliquer simplement l'interet de trier les retours avant d'en faire des taches Git; FR/EN et variantes riches synchronisees. Validation lancee apres changement.
- [2026-05-02] Ajustement couleur secondaire vitrine ter: abandon du jaune secondary au profit d'un anthracite/taupe chaud (`#72685f`) avec surfaces `#f2efec`, texte `#3f3934` et pastilles pleines en blanc pour laisser la primary bleue porter la marque. Validation lancee apres changement.
- [2026-05-02] Ajustement typographique bloc probleme: retrait de la police serif sur la section editoriale `Ca vous arrive souvent ?` pour revenir a la typographie sans-serif du site, avec graisses ajustees sur le titre, les guillemets et les citations. Validation lancee apres changement.
- [2026-05-02] Ajustement couleur secondaire vitrine bis: la teinte jaune-vert `#e6e66e` etait encore trop verte; passage a un jaune plus franc mais doux (`#eadf58`) avec fond `#fff9cf` et texte `#5c5000` pour separer accents, surfaces et textes sans casser le contraste. Validation lancee apres changement.
- [2026-05-02] Ajustement couleur secondaire vitrine: remplacement du teal par un jaune-vert acide et delave (`#e6e66e`) avec variantes soft/border/ink et texte sombre sur les pastilles pleines pour eviter l'effet securite tout en gardant le contraste. Validation lancee apres changement.
- [2026-05-02] Micro-copy hero vitrine: le paragraphe secondaire du premier pli est raccourci en FR/EN pour simplement signaler que l'app est en developpement et que les inscrits seront prevenus a l'ouverture des premiers acces. Validation lancee apres changement.
- [2026-05-02] Refonte DA section Pourquoi ChangeThis: equipe UX/UI 2026 engagee (agents Parfit et Archimedes), remplacement du traitement cartes par une section editoriale `Ca vous arrive souvent ?` avec citations, guillemets typographiques, filets fins et microcopy FR/EN plus directe. Validation lancee apres changement.
- [2026-05-02] Correctif retour ligne waitlist: le style `strong` du header de formulaire est maintenant limite au titre direct, afin que l'accent du paragraphe (`les premiers accès seront prêts`) reste inline sans couper apres `quand`. Validation lancee apres changement.
- [2026-05-02] Discipline couleurs vitrine: suppression des accents decoratifs coral/amber/success restants dans les sections marketing (`Product Flow`, pins, checks Git, badges mockup mobile) au profit de primary/secondary; les couleurs semantiques globales de l'app restent disponibles hors vitrine. Validation lancee apres changement.
- [2026-05-02] Correctif i18n hero preview: la maquette des étapes du hero `MarketingConsolePreview` n'est plus codée en dur en français; ajout des clés FR/EN pour les titres, libellés widget, feedbacks et actions. Validation lancee apres changement.
- [2026-05-02] UX section Pourquoi ChangeThis: transformation des trois constats en mini-scenes concretes avec source, realite observee, consequence et cartes plus lisibles pour faire comprendre que ce sont des exemples de terrain. Validation lancee apres changement.
- [2026-05-02] Hierarchie typo section mobile vitrine: titre mobile scinde en amorce calme et phrase d'impact (`Le contexte doit suivre` / `The context should follow`) avec tailles distinctes et responsive simple. Validation lancee apres changement.
- [2026-05-02] Micro-copy hero vitrine: retrait de la mention `avec le bon contexte` / `with the right context` dans la phrase principale, avec nettoyage du fragment `strong` vide. Validation lancee apres changement.
- [2026-05-02] Charte vitrine: remplacement de la couleur secondaire amber par un teal distinct des oranges GitLab/alerte, decouplage des variables `warning`, et usage cible sur les pictos beta, les reperes de feedback, les onglets actifs de capture et les accents de lecture du bloc probleme. Validation lancee apres changement.
...
- [2026-05-02] Préparation branche `codex/full-live-signup` hors checklist: ouverture du signup public classique avec `ENABLE_PUBLIC_SIGNUP=true`, formulaire `/signup` e-mail + mot de passe, création immédiate workspace/owner et session quand Supabase renvoie un token. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
...
- [2026-05-02] Correctif style formulaire waitlist main: bouton bêta ramené à une hauteur fixe alignée sur l'input, bordure native supprimée, focus input propre, proportions du module stabilisées et responsive mobile conservé en une colonne. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK.
- [2026-05-02] Réintégration écrans mobiles vitrine main: section mobile restaurée avec deux mockups visiteur/équipe, traitement clair aligné sur la charte, appareils redimensionnés, responsive propre et textes FR/EN ajoutés. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run build --workspace @changethis/web` OK, `git diff --check` OK, rechargement navigateur local OK.
- [2026-05-02] Ajustement écrans mobiles vitrine main: mockup équipe réaligné sur le dashboard actuel avec vues `File active`/`Historique`/`Tous`, filtres, cartes feedback compactes, badges provider/statut et synthèse `File actuelle`/`Sites connectés`. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run build --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK, capture screenshot navigateur intégrée indisponible par timeout CDP.
- [2026-05-02] Ajustement largeur vitrine main: `--home-max` élargi de 1280px à 1440px pour rapprocher le container global de la largeur applicative existante, sans étirer les blocs de texte internes. Validation lancée après changement.
- [2026-05-02] Correction copy beta Git hors checklist: suppression de la promesse fausse de priorisation Git sur la vitrine FR/EN et dans les décisions actives du carnet; formulation neutre autour des connexions GitHub et GitLab selon la configuration de chaque équipe. Validation lancée après changement.
- [2026-05-02] Enrichissement pictogrammes vitrine main: ajout de pictogrammes Lucide sur les problèmes, preuves mobiles, notes de cadre bêta et CTA final pour renforcer le scan visuel sans changer le contenu. Validation lancée après changement.
- [2026-05-02] Refonte maquette vitrine main: remplacement de l'ancienne preview interface par un élément "ChangeThis loop" en trois panneaux, couvrant installation script + connexions Git, feedback client note/pin/capture, puis inbox développeur avec actions Issue/Archiver. Validation ciblée: rechargement navigateur local OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-02] Ajustement respiration preview vitrine: suppression des pastilles numérotées `01/02/03` dans la maquette navigateur et espacement accru entre les trois panneaux pour une lecture moins compressée. Validation ciblée: rechargement navigateur local OK (`.loop-step` absent, 3 panneaux présents), `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-02] Refonte contenant preview vitrine: suppression du faux cadre navigateur `console-preview` autour du parcours produit, retrait de la barre de fenêtre, et passage des trois étapes en cartes verticales avec texte + preuve produit compacte. Validation ciblée: rechargement navigateur local OK (`.console-preview.product-loop-preview` absent, `.preview-topline` absent, 3 panneaux présents), `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-02] Micro-touche marque vitrine: découpage du wordmark header/hero en `Change` + `This`, avec `This` coloré en primaire CTA et nom conservé via `aria-label`. Validation ciblée: rechargement navigateur local OK (2 accents présents: header + hero), `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-02] Vulgarisation copy vitrine: agent copy engagé en lecture seule; remplacement des termes trop métier sur la home et la preview (`backlog`, `issue`, `inbox`, `widget`, `viewport`, `CTA`, `dashboard`, `pin`) par `bon endroit`, `tâche`, `boîte de réception`, `bouton de retour`, `taille d'écran`, `bouton`, `espace équipe`, `repère`, avec anglais aligné. Validation ciblée: rechargement navigateur local OK (`backlog`/`viewport` absents), `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-02] Accents de lecture vitrine: agent copy/UX engagé en lecture seule; ajout d'un composant `TRich` pour poser des balises `strong` ciblées dans les paragraphes clés sans HTML injecté, avec style home sobre (`color: var(--ink)`, graisse forte). Accents ajoutés sur le hero, le problème, les cartes bêta, le mobile, le workflow, le cadre bêta et le closing. Validation ciblée: rechargement navigateur local OK (strong présents dans lede/problème/cartes/étapes), `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-02] Ajustement copy problème vitrine: remplacement de `captures d'écran et messages flous` par une liste plus vivante (`e-mail, Slack, Teams, un Word, une phrase autour de la machine à café`) avec version EN alignée et accents `strong` conservés. Validation ciblée: rechargement navigateur local OK (`Teams`, `un Word`, `machine à café` présents; ancienne phrase absente), `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-02] Correctif switch langue dev: la CSP locale bloquait `unsafe-eval`, nécessaire à Next/React Refresh en développement, ce qui empêchait l'hydratation client et donc le bouton FR/EN du header. `unsafe-eval` est désormais ajouté uniquement hors production dans `next.config.ts`. Validation ciblée: header CSP local contient `unsafe-eval`, clic `EN` OK (`html lang=en`, `Available soon` visible), retour `FR` OK (`html lang=fr-BE`), `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Micro-amélioration UI dashboard hors checklist: coloration des tuiles d'erreur (`error-callout`) par sévérité (orange pour `retrying`, rouge pour `failed`) sur les cartes feedback, avec classe dédiée `error-callout--warning` / `error-callout--danger` dans `apps/web/app/styles.css`. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-02] Ajustement DA vitrine hors checklist: application plus régulière de la couleur secondary sur la home, notamment icônes produit, premier point mobile et cadre bêta, en conservant la primary pour les actions principales. Validation lancée après changement.
- [2026-05-02] Ajustement copy mobile vitrine: remplacement du titre abstrait `Le retour commence souvent sur mobile / Le contexte doit suivre` par une promesse plus directe sur les retours client qui doivent fonctionner sur chaque appareil, FR/EN alignés. Validation lancée après changement.
- [2026-05-02] Ajustement copy contexte vitrine: titre raccourci en `Le contexte arrive avec le retour.` pour rendre la promesse plus directe, avec version EN alignée. Validation lancée après changement.
- [2026-05-02] Ajustement footer vitrine: passage des textes du footer en graisse regular (`font-weight: 400`) pour la marque, liens, spans et boutons éventuels. Validation lancée après changement.
- [2026-05-02] Ajustement wording workflow vitrine: remplacement du titre par `As easy as A, B, C` en FR/EN et passage des marqueurs d'étapes de `01/02/03` à `A/B/C`. Validation lancée après changement.
- [2026-05-02] Ajustement grille contexte vitrine: ajout d'un vrai espacement responsive entre les tuiles de `Ce que contient un retour` et rayon 8px sur les cartes. Validation lancée après changement.
- [2026-05-02] Ajustement graisse typographique vitrine: passage des paragraphes narratifs de la home en regular (`font-weight: 400`), avec exceptions pour les eyebrows, statuts et phrase display du hero. Validation lancée après changement.
- [2026-05-02] Ajustement footer vitrine: simplification de la mention créateur en `mathieuluyten.be` en FR/EN. Validation lancée après changement.
- [2026-05-02] Ajustement anti-redondance vitrine: hero recentré sur l'état "en développement" et bloc waitlist recentré sur l'invitation bêta, avec FR/EN alignés. Validation lancée après changement.
- [2026-05-02] Ajustement secondary soft vitrine: remplacement de `#f6f7f6` par un off-white ciment désaturé `#f7f6f3` pour retirer la perception verte. Validation lancée après changement.
- [2026-05-02] Synchronisation UX/UI `main` vers `codex/full-live-signup`: intégration de la vitrine polie de `main`, résolution du conflit journal, et adaptation du hero/closing pour conserver les CTA `/signup` et `/login` quand `ENABLE_PUBLIC_SIGNUP=true`. Validation lancée après changement.
- [2026-05-02] Ajustement typographie globale: centralisation de la stack `Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif` dans `--font-sans` et retrait des exceptions Avenir/Georgia/monospace sur la page. Validation lancée après changement.
- [2026-05-02] Ajustement glass design vitrine: deux agents UX/UI engagés en lecture seule; application sélective du style glass Apple sur header/footer, waitlist et surfaces produit flottantes, avec textes longs et CTA primaires gardés opaques pour la lisibilité. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK, erreur CSP dev encore visible car le serveur `localhost:3000` n'a pas repris la config `unsafe-eval` déjà présente dans `next.config.ts`.
- [2026-05-02] Ajustement liquid glass CTA vitrine: application d'un traitement glass plus liquide sur les CTA de la home et les mini-CTA des mockups produit, avec CTA secondaires translucides mais actions primaires toujours très contrastées. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK, erreur CSP dev inchangée tant que le serveur `localhost:3000` n'est pas redémarré.
- [2026-05-02] Ajustement closing waitlist vitrine: retrait du pictogramme isolé au-dessus de `Liste d'attente` et réintégration en accent inline primary après `accès bêta` dans le titre FR/EN. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK (`closing-icon` absent, titre closing présent), erreur CSP dev inchangée tant que le serveur `localhost:3000` n'est pas redémarré.
- [2026-05-02] Ajustement titre closing vitrine: remplacement de `Gardez une place pour les premiers accès bêta` par une composition `<small>Réservez votre accès</small><br />dès maintenant`, avec version EN alignée et picto primary conservé comme accent inline. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK (nouveau titre présent, ancien absent), erreur CSP dev inchangée tant que le serveur `localhost:3000` n'est pas redémarré.
- [2026-05-02] Critique éditoriale home: agent éditorial engagé en lecture seule; intégration limitée aux reformulations compatibles avec les choix récents (promesse hero plus concrète, inventaire contexte moins technique, closing plus spécifique), sans toucher au closing validé ni à `Ça vous arrive souvent ?` / `As easy as A, B, C`. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK (nouvelle promesse FR et éléments protégés présents), erreur CSP dev inchangée tant que le serveur `localhost:3000` n'est pas redémarré.
- [2026-05-02] Ajustement titre cadre bêta vitrine: séparation de `Disponible prochainement,` et du fragment `avec accès volontairement progressif.` en `<small>` dédié, avec version EN alignée. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK, erreur CSP dev inchangée tant que le serveur `localhost:3000` n'est pas redémarré.
- [2026-05-02] Restauration copy hero vitrine: le paragraphe `ChangeThis est en développement. Les premiers accès ouvriront progressivement.` a été remis en FR/EN, y compris dans le rendu rich text. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK, erreur CSP dev inchangée tant que le serveur `localhost:3000` n'est pas redémarré.
- [2026-05-02] Ajustement espacement flux produit vitrine: remplacement du `gap: 1px` des tuiles `.steps` par un espacement responsive `clamp(14px, 2vw, 24px)` pour aérer les cartes. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants; rechargement navigateur local OK, erreur CSP dev inchangée tant que le serveur `localhost:3000` n'est pas redémarré.
- [2026-05-02] Micro-correction alignement waitlist vitrine: le bouton d'inscription bêta est aligné sur le champ e-mail via un offset CSS local au formulaire, avec retour mobile en pleine largeur sans décalage. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Micro-correction palette vitrine: les cartes A/B/C et le cadre bêta utilisent désormais des surfaces, bordures et icônes alignées sur les tokens primary/secondary, avec `--muted-strong` résolu sur la teinte secondary. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Micro-ajustement preview vitrine: la boucle produit de la home affiche désormais deux cartes par rangée sur desktop, avec la carte décision en pleine largeur pour préserver la lisibilité. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissement CRLF existant sur `apps/web/app/styles.css`.
- [2026-05-03] Correction structure vitrine: la preview produit est sortie du hero; le premier pli garde la colonne de gauche seule, puis une section dédiée affiche les étapes `Installer`, `Capturer`, `Trier` sur une rangée desktop. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Réarrangement hero vitrine: retrait de la grille 12 colonnes du hero autonome, largeur utile augmentée et hauteur du premier pli resserrée pour éviter l'effet de colonne isolée après déplacement de la preview. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Ajustement hero vitrine en deux zones: séparation de la promesse à gauche et de la carte invitation à droite, avec empilement mobile conservé et preview produit maintenue en section suivante. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Resserrement palette vitrine: remplacement des accents verts/success et de plusieurs surfaces/bordures bleutées par la gamme secondary sur la home, notamment waitlist, cadre bêta, cartes produit, preview boucle et sections mobiles. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Passe composants couleurs vitrine: audit des composants home (`ProviderBadge`, badges de statut, mockups mobiles, preview boucle, hero providers) et ajout de surcharges `.app-home` pour respecter primary + gris ciment sans contaminer le dashboard applicatif. Validation ciblée: aucun style inline couleur dans les TSX, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Restauration couleurs provider vitrine: retrait des surcharges qui neutralisaient GitHub/GitLab; les marques provider gardent leurs couleurs officielles, pendant que les statuts/surfaces restent alignés sur primary + gris ciment. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Ajustement graisse hero vitrine: `hero-statement` descend de 850 à 650 pour éviter l'effet bold permanent, avec GitHub/GitLab conservés plus marqués en 850. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Homogénéisation architecture vitrine: ajout d'un header de section `Parcours produit` au-dessus de la boucle `Installer / Capturer / Trier`, avec titre et texte d'introduction FR/EN pour aligner cette zone sur les autres sections. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Correction hauteur tuiles boucle produit: la grille `product-loop-board` n'étire plus les cartes à la hauteur de la rangée; chaque panneau garde désormais la hauteur de son contenu. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Ajustement alignement boucle produit: les panneaux de hauteur naturelle sont centrés verticalement dans la rangée via `align-items: center` sur `product-loop-board`. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Renforcement lecture séquentielle boucle produit: ajout de connecteurs/flèches CSS entre les panneaux `Installer`, `Capturer` et `Trier` sur desktop, masqués en layout mobile. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Désaturation fonds vitrine: remplacement des fonds bleutés/lavande résiduels (`#f7f8ff`, `#f7f9fc`, `#f8faff`) par une gamme gris ciment, retrait des saturations glass visibles et recentrage du primary sur les actions/accents. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Correction empilement connecteurs boucle produit: isolation de la grille et z-index dédié aux panneaux avec flèche pour éviter que la tuile suivante masque le connecteur. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Passe stricte couleur mauve + gris: `secondary` devient une vraie gamme RGB désaturée, suppression des verts/oranges/rouges visibles dans les statuts, provider GitLab neutralisé vers le primary mauve, fonds ciment remplacés par gris purs et seed demo aligné sur primary/gris. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Durcissement charte couleur: neutralisation des bleus-gris restants (`ink`, `muted`, `tracker`, GitHub, surfaces mockups, lignes et widget fallback) pour ne garder dans les sources que la gamme primary mauve et des gris RGB désaturés. Validation ciblée: scan hex source OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Audit agents + garde-fou couleur: agents UX/UI engagés en lecture seule; primary conservée sur la rampe existante, suppression des `rgba` chromatiques hors primary/gris, retrait des `color-mix`, neutralisation de `--soft`, et ajout de `npm run color:check` pour bloquer toute couleur hors primary ou gris RGB désaturé. Validation ciblée: `npm run color:check` OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Correction dérive primary: annulation de la tentative de remplacement de la primary par `#6f4bb3`; la rampe primary initiale (`#3f51b5` et variantes) est restaurée, tout en gardant le garde-fou couleur et les gris secondaires désaturés. Validation ciblée: `npm run color:check` OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Restauration couleurs provider: GitHub/GitLab gardent leurs couleurs de marque officielles (`#24292f`, `#eaecef`, `#fc6d26`, `#fff0e8`, bordures associées) comme exceptions explicites au garde-fou couleur; les surcharges home ne neutralisent plus les badges provider mobiles. Validation ciblée: `npm run color:check` OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Correction visibilité connecteurs boucle produit: remplacement des flèches attachées aux cartes par deux flèches portées par `product-loop-board` en overlay de grille, masquées sous 1080px, pour éviter qu'une carte recouvre le connecteur. Validation ciblée: `npm run color:check` OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-03] Passe redesign Figma trends 2026 vitrine: agents DA/UX, design-system et frontend/perf engagés; ajout de tokens home tactiles, profondeur légère, micro-motions accessibles, surfaces harmonisées et `prefers-reduced-motion`, sans changer la primary ni neutraliser GitHub/GitLab. Validation ciblée: `npm run color:check` OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Passe couleur Figma combinations vitrine: agents couleur/design-system/a11y engagés; primary réaffirmée comme fil de marque/action/état actif (waitlist, icônes produit, onglets actifs, pin, feedback actif, numérotation du parcours), gris conservé pour les surfaces et providers GitHub/GitLab gardés en exceptions brand. Validation ciblée: `npm run color:check` OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Micro-correction section mobile vitrine: titre remplacé par `ChangeThis garde chaque retour exploitable`, paragraphe resserré et espacement explicite ajouté entre le titre et le texte pour éviter la collision visuelle. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run color:check` OK.
- [2026-05-03] Micro-correction closing vitrine: déplacement de l'icône Sparkles après le fragment `dès maintenant` / `now` dans le titre de closing. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Micro-correction pastille mobile vitrine: `Visiteur sans compte` passe en primary doux avec barre interne et ombre légère pour lire comme un point mis en avant plutôt qu'un état disabled. Validation ciblée: `npm run color:check` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-03] Ajustement fonds vitrine: augmentation du blanc pur sur les grands backgrounds de la home, de la section mobile, du canvas parcours et des conteneurs waitlist/bêta, en gardant le gris ciment pour les lignes, grilles et surfaces secondaires. Validation ciblée: `npm run color:check` OK, `git diff --check` OK avec avertissement CRLF existant.
- [2026-05-04] Ajout section institutions publiques sur la home vitrine: positionnement B2G sobre autour d'un canal usager accessible, traçable et actionnable, avec thèmes accessibilité continue, dossier sécurité/RGPD, pilote contrôlé, limites explicites et cadrage pilote FR/EN. Validation ciblée: `npm run color:check` OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Micro-correction waitlist vitrine: simplification des points sous le formulaire en deux messages directs (`Inscrivez-vous à la waiting list dès maintenant.` / `Pensé pour GitHub ou GitLab.`) avec version EN alignée. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Micro-correction cadre bêta vitrine: remplacement des notes longues par trois messages concrets sur visiteur sans compte, repos/projets liés dans le dashboard et fonctionnement desktop/mobile/tablette, avec version EN alignée. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Micro-correction titre cadre bêta: retrait du sous-titre `avec accès volontairement progressif.` pour ne garder que `Disponible prochainement.` / `Available soon.`. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Micro-correction typo FR vitrine: ajout d'espaces insécables avant les ponctuations françaises fortes visibles sur la home (`Bêta privée\u00a0:`, `Ça vous arrive souvent\u00a0?`, `signal incomplet\u00a0:`, `contexte exact\u00a0:`, `cas\u00a0:`), y compris dans le rendu rich text. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Réécriture section institutions publiques vitrine: copy simplifiée autour de trois besoins concrets (moins de retours perdus, savoir quoi corriger, garder une trace), avec limites et démarrage pilote raccourcis. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Micro-correction intro parcours produit: remplacement du titre explicatif par `Un flux simple.` et du texte par `Un script à placer sur le site en dev. C'est tout.`, avec version EN alignée. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Repositionnement section institutions publiques: retrait de l'angle critique du service public au profit des garanties d'évaluation IT/DPO (token Git hors script/navigateur, cadrage RGPD documentable, captures configurables, périmètre pilote). Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run color:check` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Ajustement carte `Capturer` parcours produit: remplacement du titre long par une intro et une liste ordonnée iconée Lucide (`page`, `zone précise`, `épingle précise`) dans la preview, avec version EN alignée. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run color:check` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Ajustement réassurance IT/DPO vitrine: retrait des mentions visibles `Institutions publiques` / `service public` de la section pour garder une posture générale de validation interne compatible institutions sans les cibler frontalement. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Ajustement hiérarchie carte `Capturer`: l'intro devient un kicker discret et les trois options de feedback deviennent le contenu visuel principal, avec taille/graisse renforcées et icônes Lucide conservées. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run color:check` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Raccourcissement architecture vitrine: suppression des sections autonomes `Ce que fait ChangeThis`, workflow et cadre bêta, déplacement du problème avant le parcours produit, réduction du contexte à 4 preuves et allègement de la section mobile en preuve visuelle. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run color:check` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Port UX/UI vers `codex/full-live-signup`: cherry-pick des ajustements récents de `main` après la branche d'hier, avec conservation des CTA `/signup` et `/login` quand `ENABLE_PUBLIC_SIGNUP=true`. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `npm run color:check` OK, `git diff --check` OK avec avertissement CRLF existant.
- [2026-05-04] Ajustement row feedback `/projects`: suppression de l'overlay visuel qui masquait la capture dans le bouton de prévisualisation (`ScreenshotPreview`), réorganisation de la cellule “Issue” en conteneur dédié (destination + mini-prévisualisation) pour clarifier l'architecture de ligne sans éléments superposés; validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, vérification visuelle via navigateur local.
- [2026-05-04] Micro-correction header vitrine: affichage explicite des boutons `Connexion` et `S'inscrire` dans le header de la home, avec lien `/signup` conservé même quand l'inscription publique renvoie l'état bêta privée. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK; `npm run color:check` bloqué par couleurs existantes dans `apps/web/app/styles.css` (`#fff7e6`, `#ad4f00`, `#fdecea`, etc.) hors fichiers modifiés pour cette demande.
- [2026-05-04] Micro-correction row feedback `/projects`: la pastille provider de la cellule “Issue” n'est plus étirée par la grille au-dessus de la capture; `feedback-issue-destination` aligne désormais ses éléments en largeur naturelle et la mini-prévisualisation reste ancrée à gauche. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-04] Sélection de masse dashboard `/projects`: ajout d'un `select all` dans la première cellule du header du tableau et d'un miroir `Tout sélectionner / Tout désélectionner` dans la toolbar bulk pour les vues mobiles où le header disparaît. Le contrôle agit uniquement sur les feedbacks affichés et sélectionnables, avec état indéterminé et compteur `X sur Y affichés`. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-04] Revue UX/UI ciblée des rows dashboard via agent explorateur: principaux points relevés à traiter dans une prochaine passe atomique: hiérarchie visuelle trop plate par duplication des métadonnées, cellule `Issue` encore ambiguë entre état/routage/preuve, actions trop saillantes face au signal principal, et simplification mobile incomplète. Recommandation minimale: recentrer la row sur `signal principal / contexte court / traitement`, puis reléguer le reste dans le disclosure.
- [2026-05-04] Micro-correction ordre cellule `Issue` dashboard: la capture est désormais rendue avant le bloc provider/état (`GitHub` + `Non créée`) pour éviter l'effet de bouton Git au-dessus de la preuve visuelle; le bloc destination est aussi compacté sur une seule ligne flexible. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-04] Correction structurelle row dashboard: séparation réelle de la capture et de l'issue en deux colonnes distinctes (`Capture` puis `Issue`) dans le tableau `/projects`; la miniature n'est plus dans `feedback-issue-cell`, donc le badge GitHub/GitLab ne peut plus se placer au-dessus ou au-dessous du bouton capture. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK avec avertissements CRLF existants.
- [2026-05-04] Simplification miniature capture dashboard: retrait du libellé `Capture` et de la taille fichier dans la row; le bouton affiche uniquement la miniature avec un masque noir semi-transparent et l'icône œil centrée. La taille reste disponible dans la modale de détail. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-04] Agrandissement miniature capture dashboard: le bouton de prévisualisation occupe désormais toute la largeur disponible de la cellule `Capture` et s'étire sur la hauteur utile de la row, avec image en `object-fit: cover` sous le masque noir et l'icône œil centrée. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK.
- [2026-05-19] Contournement SafeLinks/CRA signup: remplacement du signup par lien consommable par un code e-mail à recopier (`/auth/v1/otp` + `/auth/v1/verify` avec `email/token/type=email`), partage du helper cookies session, et mise à jour du template Supabase vers `Magic Link` avec `{{ .Token }}`. Validation ciblée: `npm run test --workspace @changethis/web` OK, `npm run typecheck --workspace @changethis/web` OK.
- [2026-05-19] Fine tuning états actifs UI: ajout d'un token `--active-control-shadow` façon shadow-sm Preline et application aux contrôles segmentés/tabs actifs, notamment le switch FR/EN, pour retirer l'ombre inset trop marquée sans toucher aux cartes actives métier. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK après correction du setState sync dans `/auth/confirm`, `git diff --check` OK.
- [2026-05-19] Correction session signup/logout: `/signup` redirige désormais les utilisateurs déjà connectés vers `/projects` ou `/signup/set-password`, y compris dans les server actions, et `/logout` répond en `303` avec expiration explicite des cookies pour forcer la navigation post-déconnexion. Validation ciblée: `npm run test --workspace @changethis/web` OK, `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK.
- [2026-05-19] Correction design gestion utilisateurs: la table utilisateurs utilise désormais 5 colonnes réelles pour aligner `Arrivée`, et le formulaire d'invitation est groupé/stylé avec champs cohérents dashboard au lieu de contrôles natifs serrés. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK.
- [2026-05-19] Ajustement slogan footer: remplacement de la mention bêta par une promesse production durable en FR/EN (`Transformez chaque retour client...`). Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK.
- [2026-05-19] Ajustement wording bêta ouverte: retrait des mentions visibles `contrôlée` / accès progressifs sur le hero et les preuves FR/EN, pour refléter une bêta réellement ouverte sans garde-fou manuel. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK.
- [2026-05-19] Simplification dashboard 2 colonnes: suppression des panneaux `Checklist activation` et `Activité`, déplacement de la synthèse sous `Sites connectés`, et retour du workbench `/projects` à deux colonnes pour préserver la largeur utile de la file. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web` et `git diff --check`.
- [2026-05-19] Correction ombrage CTA globaux: normalisation des bordures/focus des `.button` et `.danger-button` pour éviter les contours navigateur et halos actifs trop marqués sur `Enregistrer` / `Déconnecter`. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web` et `git diff --check`.
- [2026-05-19] Correction formulaire tokens Git: le formulaire token serveur reste visible même si un provider est marqué désactivé, pour permettre de recoller un token et réactiver GitHub/GitLab; consigne GitLab alignée sur Project access token avec rôle Reporter/Developer et scope `api`. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web` et `git diff --check`.
- [2026-05-19] Finition UI widget feedback: retrait du pictogramme de marque dans le header, remplacement de `Découvrir` par une icône d'aide, déplacement du bouton `Contact` dans les actions header, suppression du texte résumé qui débordait, et rappel primary sur panneau/onglet actif/bouton d'envoi. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web`, `npm test --workspace @changethis/widget`, `npm run build --workspace @changethis/widget` et `git diff --check`.
- [2026-05-19] Correction auth GitLab Project Access Token: les appels de listing GitLab n'envoient plus `Authorization: Bearer` en parallèle de `PRIVATE-TOKEN`, pour éviter les 401 avec Project Access Token; métadonnée scopes sauvegardée alignée sur `api` seul. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web`, `npx tsx --test test/issue-providers.test.ts` et `git diff --check`.
- [2026-05-19] Ajustement timeout providers Git: timeout par défaut des appels GitHub/GitLab relevé à 30s pour éviter les faux échecs `gitlab request timed out` lors du listing de projets depuis Railway; `ISSUE_PROVIDER_TIMEOUT_MS` reste configurable. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web`, `npx tsx --test test/issue-providers.test.ts` et `git diff --check`.
- [2026-05-19] Correction UX déconnexion: les liens `Se déconnecter` du header/footer pointent vers `/logout` en navigation GET complète, afin de vider les cookies, recharger la page et rediriger vers `/login`; ajout d'un test GET logout. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web`, `npx tsx --test test/logout-route.test.ts` et `git diff --check`.
- [2026-05-19] Correction parcours GitLab Project Access Token: ajout d'une validation de dépôt par URL via `/projects/:id` pour les tokens scoped à un seul projet, ajout d'un champ URL du dépôt cible dans la création de site, et conservation du listing uniquement quand il est disponible. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web`, `npx tsx --test test/issue-providers.test.ts` et `git diff --check`.
- [2026-05-19] Support GitLab self-hosted par URL: les URLs de dépôt GitLab HTTPS hors `gitlab.com` sont acceptées pour la création de site, la validation utilise l'origine du dépôt (`/api/v4/projects/:id`) et les issues sont créées sur cette même origine via le `webUrl` stocké. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web`, `npx tsx --test test/issue-providers.test.ts` et `npx tsx --test test/project-registry.test.ts`.
- [2026-05-19] Alignement GitLab sur GitHub via Personal Access Token: ajout d'un champ instance GitLab au formulaire de connexion, sauvegarde du `base_url` provider avec le token, listing des projets depuis cette instance, copy UI orientée Personal Access Token `api` et documentation locale mise à jour. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web`, `npx tsx --test test/issue-providers.test.ts`, `npx tsx --test test/project-registry.test.ts` et `git diff --check`.
- [2026-05-19] Correction validation token Git: la connexion serveur vérifie désormais le listing des dépôts avant de confirmer un token, désactive l'intégration si la vérification échoue, et le toast de succès n'est affiché que si le refresh UI retourne un état actif. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web`, `npx tsx --test test/issue-providers.test.ts`, `npx tsx --test test/project-registry.test.ts` et `git diff --check`.
- [2026-05-19] Correction origine logout: `/logout` utilise désormais `NEXT_PUBLIC_APP_URL` pour construire la redirection publique quand l'URL entrante vient de `https://localhost:8080`, avec fallback sur l'URL de requête en local non configuré. Validé par `npx tsx --test test/logout-route.test.ts`, `npm run typecheck --workspace @changethis/web` et `npm run lint --workspace @changethis/web`.
- [2026-05-19] Assouplissement connexion GitLab token limité: l'enregistrement du token GitLab ne bloque plus quand le listing global `/api/v4/projects` renvoie 401, afin de conserver le flux Project Access Token ou token limité avec URL du dépôt cible dans Sites connectés. Validé par `npm run typecheck --workspace @changethis/web`, `npm run lint --workspace @changethis/web` et `npx tsx --test test/issue-providers.test.ts`.
- [2026-05-19] Correction warnings Supabase Security Advisor: ajout de la migration `0012` révoquant `EXECUTE` sur les helpers RLS `is_organization_member` et `can_manage_organization` pour `public`/`anon`/`authenticated`, alignement du SQL consolidé et du check migrations. Validation ciblée: `npm run migrations:check` OK.
- [2026-05-19] Homogénéisation inputs tokens Git: les champs Personal Access Token et instance GitLab ont désormais une surface non focusée explicite, alignée sur les contrôles dashboard, avec focus/autofill/disabled cohérents. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK; `npm run color:check` reste bloqué par hex existants hors changement.
- [2026-05-19] Réordonnancement création site connecté: le formulaire modal suit désormais l'ordre provider Git, dépôt, nom projet auto depuis le repo, URL du site puis options de placement widget; l'API de création reçoit aussi les options widget initiales. Validation ciblée: `npm run typecheck --workspace @changethis/web` OK, `npm run lint --workspace @changethis/web` OK, `git diff --check` OK; vérification navigateur automatisée indisponible faute d'outil Node/browser exposé.
